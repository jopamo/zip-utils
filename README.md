...
- `zipnote`, `zipcloak`, and `zipsplit` are recognized aliases but remain minimal (zipnote supports comment editing via `-w`; no re-encryption; split read/write is supported but disk metadata is simplified).
- Archive comments supported via `zip -z` (including comment-only edits) and preserved on rewrite; entry comments are editable via `zipnote -w` and preserved during rewrites.
