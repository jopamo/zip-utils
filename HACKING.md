...
* Zip baseline: `python3 document_zip_output.py --zip zip --outdir zip-doc-system` (detects the system Info-ZIP Zip 3.0, runs 38 scenarios, writes logs + `summary.md`/JSONL into `zip-doc-system/`)
* Zip comparison: `python3 document_zip_output.py --zip /home/me/projects/zip-utils/build/zip --outdir zip-doc-build` (use an absolute path so temp working dirs do not break relative paths; runs the same 38 scenarios and writes into `zip-doc-build/`)
* Unzip baseline: `python3 document_unzip_output.py --unzip unzip --outdir unzip-doc-system` (detects the system Info-ZIP UnZip 6.00, runs 20 scenarios, writes logs + `summary.md`/JSONL into `unzip-doc-system/`)
* Unzip comparison: `python3 document_unzip_output.py --unzip /home/me/projects/zip-utils/build/unzip --outdir unzip-doc-build` (absolute path avoids temp dir surprises; runs the same 20 scenarios and writes into `unzip-doc-build/`)
* Zipinfo baseline: `python3 document_zipinfo_output.py --zipinfo zipinfo --outdir zipinfo-doc-system` (detects system zipinfo; runs zipinfo-format scenarios and writes logs + `summary.md`/JSONL into `zipinfo-doc-system/`)
* Zipinfo comparison: `python3 document_zipinfo_output.py --zipinfo /home/me/projects/zip-utils/build/unzip --zipinfo-arg=-Z --outdir zipinfo-doc-build` (use `-Z` when invoking through the unzip binary; runs the same scenarios and writes into `zipinfo-doc-build/`)
