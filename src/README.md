...
- `src/format/`: archive reader/writer implementations (central directory handling, Zip64, streaming).
...
- Writer (`src/format/writer.c`) owns create/update flows, copy-on-write output (`-O`), ZipCrypto, and streaming stdin/stdout via data descriptors when the path is `-` (split volumes are not supported).
