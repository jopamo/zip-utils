...
static int read_ux3_value OF((ZCONST uch * dbuf, unsigned uidgid_sz, ulg* p_uidgid));
#endif /* IZ_HAVE_UXUIDGID */

static ZCONST char Far CannotAllocateBuffers[] = "error:  cannot allocate unzip buffers\n";

#ifdef SFX
static ZCONST char Far CannotFindMyself[] = "unzipsfx:  cannot find myself! [%s]\n";
#ifdef CHEAP_SFX_AUTORUN
static ZCONST char Far AutorunPrompt[] = "\nAuto-run command: %s\nExecute this command? [y/n] ";
static ZCONST char Far NotAutoRunning[] = "Not executing auto-run command.";
#endif
...
#endif /* DO_SAFECHECK_2GB */
static ZCONST char Far MaybeExe[] = "note:  %s may be a plain executable, not an archive\n";
...
#else  /* SFX */
static ZCONST char Far CentDirEndSigNotFound[] = "  End-of-central-directory signature not found.\n";
#endif /* ?SFX */
#ifdef TIMESTAMP
static ZCONST char Far ZipTimeStampFailed[] = "warning:  cannot set time for %s\n";
...
int process_zipfiles(__G) /* return PK-type error code */
    ...
    if ((G.inbuf == (uch*)NULL) || (G.outbuf == (uch*)NULL)) {
        Info(slide, 0x401, ((char*)slide, LoadFarString(CannotAllocateBuffers)));
        return (PK_MEM);
    }
...
/* Initialize UnZip's built-in pseudo hard-coded "ISO <--> OEM" translation,
   depending on the detected codepage setup.  */
    ...
    /*---------------------------------------------------------------------------
        ...
        precedence.
      ---------------------------------------------------------------------------*/
    G.overwrite_mode = (uO.overwrite_none ? OVERWRT_NEVER : (uO.overwrite_all ? OVERWRT_ALWAYS : OVERWRT_QUERY));
    ...
    if ((error = do_seekable(__G__ 0)) == PK_NOZIP) {
        ...
        if ((G.zipfn = (char*)malloc(len + sizeof(EXE_EXTENSION))) != (char*)NULL) {
    ...
    if (error) {
        ...
        if (error == PK_NOZIP)
            Info(slide, 1, ((char*)slide, LoadFarString(CannotFindMyself), G.zipfn));
    }
#ifdef CHEAP_SFX_AUTORUN
    if (G.autorun_command[0] && !uO.qflag) { /* NO autorun without prompt! */
        Info(slide, 0x81, ((char*)slide, LoadFarString(AutorunPrompt), FnFilter1(G.autorun_command)));
        if (fgets(G.answerbuf, 9, stdin) != (char*)NULL && toupper(*G.answerbuf) == 'Y')
    ...
    if (iswild(G.wildzipfn) && uO.qflag < 3
        ...
        if ((NumWinFiles > 1) || (NumWinFiles == 1 && NumMissDirs + NumMissFiles + NumLoseFiles + NumWarnFiles > 0))
            Info(slide, 0x401, ((char*)slide, LoadFarString(FilesProcessOK), NumWinFiles, (NumWinFiles == 1) ? " was" : "s were"));
        if (NumWarnFiles > 0)
            Info(slide, 0x401, ((char*)slide, LoadFarString(ArchiveWarning), NumWarnFiles, (NumWarnFiles == 1) ? "" : "s"));
        if (NumLoseFiles > 0)
            Info(slide, 0x401, ((char*)slide, LoadFarString(ArchiveFatalError), NumLoseFiles, (NumLoseFiles == 1) ? "" : "s"));
        if (NumMissFiles > 0)
            Info(slide, 0x401, ((char*)slide, LoadFarString(FileHadNoZipfileDir), NumMissFiles, (NumMissFiles == 1) ? "" : "s"));
        if (NumMissDirs == 1)
            Info(slide, 0x401, ((char*)slide, LoadFarString(ZipfileWasDir)));
        else if (NumMissDirs > 0)
            Info(slide, 0x401, ((char*)slide, LoadFarString(ManyZipfilesWereDir), NumMissDirs));
...
" We need a better error message for: 64-bit file, 32-bit program.\n");
...
#else
        if (maybe_exe)
            Info(slide, 0x401, ((char*)slide, LoadFarString(MaybeExe), G.zipfn));
        if (lastchance)
            return error_in_archive;
        else {
...
#ifdef NO_MULTIPART
    error = !uO.zipinfo_mode && (G.ecrec.number_this_disk == 1) && (G.ecrec.num_disk_start_cdir == 1);
#else
    ...
    if (uO.zipinfo_mode && G.ecrec.number_this_disk != G.ecrec.num_disk_start_cdir) {
        if (G.ecrec.number_this_disk > G.ecrec.num_disk_start_cdir) {
            Info(slide, 0x401, ((char*)slide, LoadFarString(CentDirNotInZipMsg), G.zipfn, (ulg)G.ecrec.number_this_disk, (ulg)G.ecrec.num_disk_start_cdir));
            error_in_archive = PK_FIND;
            too_weird_to_continue = TRUE;
        }
        else {
            Info(slide, 0x401, ((char*)slide, LoadFarString(EndCentDirBogus), G.zipfn, (ulg)G.ecrec.number_this_disk, (ulg)G.ecrec.num_disk_start_cdir));
        ...
        Info(slide, 0x401, ((char*)slide, LoadFarString(NoMultiDiskArcSupport), G.zipfn));
        error_in_archive = PK_FIND;
        too_weird_to_continue = TRUE;
#endif
    }
    ...
    if (!too_weird_to_continue) { /* (relatively) normal zipfile:  go for it */
        if (error) {
            Info(slide, 0x401, ((char*)slide, LoadFarString(MaybePakBug), G.zipfn));
            error_in_archive = PK_WARN;
        }
#endif /* !SFX */
        if ((G.extra_bytes = G.real_ecrec_offset - G.expect_ecrec_offset) < (zoff_t)0) {
            Info(slide, 0x401, ((char*)slide, LoadFarString(MissingBytes), G.zipfn, FmZofft((-G.extra_bytes), NULL, NULL)));
            error_in_archive = PK_ERR;
        }
        else if (G.extra_bytes > 0) {
            if ((G.ecrec.offset_start_central_directory == 0) && (G.ecrec.size_central_directory != 0)) /* zip 1.5 -go bug */
            {
                Info(slide, 0x401, ((char*)slide, LoadFarString(NullCentDirOffset), G.zipfn));
                ...
                Info(slide, 0x401, ((char*)slide, LoadFarString(ExtraBytesAtStart), G.zipfn, FmZofft(G.extra_bytes, NULL, NULL), (G.extra_bytes == 1) ? "" : "s"));
                error_in_archive = PK_WARN;
            }
#endif /* !SFX */
        }
        ...
        if (G.expect_ecrec_offset == 0L && G.ecrec.size_central_directory == 0) {
            if (uO.zipinfo_mode)
                ...
                Info(slide, 0x401, ((char*)slide, LoadFarString(ZipfileEmpty), G.zipfn));
            CLOSE_INFILE();
...
#else
    if ((error != PK_OK) || (readbuf(__G__ G.sig, 4) == 0) || memcmp(G.sig, central_hdr_sig, 4))
#endif
        {
#ifndef SFX
            ...
            if ((error != PK_OK) || (readbuf(__G__ G.sig, 4) == 0) || memcmp(G.sig, central_hdr_sig, 4)) {
                if (error != PK_BADERR)
                    Info(slide, 0x401, ((char*)slide, LoadFarString(CentDirStartNotFound), G.zipfn, LoadFarStringSmall(ReportMsg)));
                CLOSE_INFILE();
                return (error != PK_OK ? error : PK_BADERR);
            }
#ifndef SFX
            Info(slide, 0x401, ((char*)slide, LoadFarString(CentDirTooLong), G.zipfn, FmZofft((-tmp), NULL, NULL)));
#endif
            error_in_archive = PK_ERR;
        }
    ...
    if (uO.T_flag && !uO.zipinfo_mode && (nmember > 0L)) {
        if (stamp_file(G.zipfn, uxstamp)) { /* TIME-STAMP 'EM */
            ...
            if (!uO.qflag)
                Info(slide, 0, ((char*)slide, LoadFarString(ZipTimeStampSuccess), G.zipfn));
        }
    }
#endif
    return error_in_archive;
...
#endif /* ?USE_STRM_INPUT */
        if ((G.incnt = read(G.zipfd, (char*)G.inbuf, (unsigned int)tail_len)) != (int)tail_len)
            return 2; /* it's expedient... */

        /* 'P' must be at least (rec_size+4) bytes from end of zipfile */
        for (G.inptr = G.inbuf + (int)tail_len - (rec_size + 4); G.inptr >= G.inbuf; --G.inptr) {
...
static int find_ecrec64(__G__) {
    ...
    if ((G.incnt = read(G.zipfd, (char*)byterecL, ECLOC64_SIZE + 4)) != (ECLOC64_SIZE + 4)) {
        ...
        Info(slide, 0x401, ((char*)slide, LoadFarString(Cent64EndSigSearchErr)));
        return PK_ERR;
    }
    ...
    if (memcmp((char*)byterec, end_central64_sig, 4)) {
        ...
        if (uO.qflag || uO.zipinfo_mode)
            Info(slide, 0x401, ((char*)slide, "[%s]\n", G.zipfn));
        Info(slide, 0x401, ((char*)slide, LoadFarString(Cent64EndSigSearchOff)));
    }

    /* Check consistency of found ecrec64 with ecloc64 (and ecrec): */
    if ((zuvl_t)makelong(&byterec[NUMBER_THIS_DSK_REC64]) != ecrec64_start_disk)
     ...
     */
    ecrec64_disk_cdstart = (zuvl_t)makelong(&byterec[NUM_DISK_START_CEN_DIR64]);
    if ((G.ecrec.num_disk_start_cdir != 0xFFFF) && (G.ecrec.num_disk_start_cdir != ecrec64_disk_cdstart))
        return PK_COOL;
    ecrec64_this_entries = makeint64(&byterec[NUM_ENTRIES_CEN_DIR_THS_DISK64]);
    if ((G.ecrec.num_entries_centrl_dir_ths_disk != 0xFFFF) && (G.ecrec.num_entries_centrl_dir_ths_disk != ecrec64_this_entries))
        return PK_COOL;
    ecrec64_tot_entries = makeint64(&byterec[TOTAL_ENTRIES_CENTRAL_DIR64]);
    if ((G.ecrec.total_entries_central_dir != 0xFFFF) && (G.ecrec.total_entries_central_dir != ecrec64_tot_entries))
        return PK_COOL;
    ecrec64_cdirsize = makeint64(&byterec[SIZE_CENTRAL_DIRECTORY64]);
    if ((G.ecrec.size_central_directory != 0xFFFFFFFFL) && (G.ecrec.size_central_directory != ecrec64_cdirsize))
        return PK_COOL;
    ecrec64_offs_cdstart = makeint64(&byterec[OFFSET_START_CENTRAL_DIRECT64]);
    if ((G.ecrec.offset_start_central_directory != 0xFFFFFFFFL) && (G.ecrec.offset_start_central_directory != ecrec64_offs_cdstart))
        return PK_COOL;
    ...
    G.ecrec.ec64_end = ecrec64_start_offset + 12 + makeint64(&byterec[ECREC64_LENGTH]);
...
static int find_ecrec(__GPRO__ zoff_t searchlen) /* return PK-class error */
{
    ...
    if (G.ziplen <= INBUFSIZ) {
...
#endif /* ?USE_STRM_INPUT */
        if ((G.incnt = read(G.zipfd, (char*)G.inbuf, (unsigned int)G.ziplen)) == (int)G.ziplen)

            /* 'P' must be at least (ECREC_SIZE+4) bytes from end of zipfile */
            for (G.inptr = G.inbuf + (int)G.ziplen - (ECREC_SIZE + 4); G.inptr >= G.inbuf; --G.inptr) {
                if ((*G.inptr == (uch)0x50) && /* ASCII 'P' */
        ...
        found = (rec_find(__G__ searchlen, end_central_sig, ECREC_SIZE) == 0 ? TRUE : FALSE);
    ...
    if (!found) {
        if (uO.qflag || uO.zipinfo_mode)
            Info(slide, 0x401, ((char*)slide, "[%s]\n", G.zipfn));
        Info(slide, 0x401, ((char*)slide, LoadFarString(CentDirEndSigNotFound)));
...
#ifdef TEST
    printf("\n  found end-of-central-dir signature at offset %s (%sh)\n", FmZofft(G.real_ecrec_offset, NULL, NULL), FmZofft(G.real_ecrec_offset, FZOFFT_HEX_DOT_WID, "X"));
    ...
    G.ecrec.num_disk_start_cdir = makeword(&byterec[NUM_DISK_WITH_START_CEN_DIR]);
    G.ecrec.num_entries_centrl_dir_ths_disk = makeword(&byterec[NUM_ENTRIES_CEN_DIR_THS_DISK]);
    G.ecrec.total_entries_central_dir = makeword(&byterec[TOTAL_ENTRIES_CENTRAL_DIR]);
    G.ecrec.size_central_directory = makelong(&byterec[SIZE_CENTRAL_DIRECTORY]);
    G.ecrec.offset_start_central_directory = makelong(&byterec[OFFSET_START_CENTRAL_DIRECTORY]);
    ...
    G.expect_ecrec_offset = G.ecrec.offset_start_central_directory + G.ecrec.size_central_directory;
...
static int process_zip_cmmnt(__G) /* return PK-type error code */
    ...
    if (uO.zipinfo_mode && uO.lflag > 9) {
        ...
        if (!G.ecrec.zipfile_comment_length)
            ...
            Info(slide, 0, ((char*)slide, LoadFarString(ZipfileCommentDesc), G.ecrec.zipfile_comment_length));
            ...
            if (error)
                Info(slide, 0, ((char*)slide, LoadFarString(ZipfileCommTrunc2)));
        } /* endif (comment exists) */

        /* ZipInfo, non-verbose mode:  print zipfile comment only if requested */
    }
    else if (G.ecrec.zipfile_comment_length && (uO.zflag > 0) && uO.zipinfo_mode) {
        if (do_string(__G__ G.ecrec.zipfile_comment_length, DISPLAY)) {
            Info(slide, 0x401, ((char*)slide, LoadFarString(ZipfileCommTrunc1)));
            error = PK_WARN;
        }
    }
    else
...
int process_cdir_file_hdr(__G) /* return PK-type error code */
    ...
    if (IS_VOLID(G.crec.external_file_attributes) && (G.pInfo->hostnum == FS_FAT_ || G.pInfo->hostnum == FS_HPFS_ || G.pInfo->hostnum == FS_NTFS_ || G.pInfo->hostnum == ATARI_)) {
    ...
    G.pInfo->GPFIsUTF8 = (G.crec.general_purpose_bit_flag & (1 << 11)) == (1 << 11);
#endif
...
static int get_cdir_ent(__G) /* return PK-type error code */
    ...
    G.crec.version_needed_to_extract[0] = byterec[C_VERSION_NEEDED_TO_EXTRACT_0];
    G.crec.version_needed_to_extract[1] = byterec[C_VERSION_NEEDED_TO_EXTRACT_1];

    G.crec.general_purpose_bit_flag = makeword(&byterec[C_GENERAL_PURPOSE_BIT_FLAG]);
    ...
    G.crec.internal_file_attributes = makeword(&byterec[C_INTERNAL_FILE_ATTRIBUTES]);
    G.crec.external_file_attributes = makelong(&byterec[C_EXTERNAL_FILE_ATTRIBUTES]); /* LONG, not word! */
    G.crec.relative_offset_local_header = makelong(&byterec[C_RELATIVE_OFFSET_LOCAL_HEADER]);
...
int process_local_file_hdr(__G) /* return PK-type error code */
    ...
    G.lrec.version_needed_to_extract[0] = byterec[L_VERSION_NEEDED_TO_EXTRACT_0];
    G.lrec.version_needed_to_extract[1] = byterec[L_VERSION_NEEDED_TO_EXTRACT_1];

    G.lrec.general_purpose_bit_flag = makeword(&byterec[L_GENERAL_PURPOSE_BIT_FLAG]);
...
unsigned ef_len;    /* total length of extra field */
{
    ...
    while (ef_len >= EB_HEADSIZE) {
        ...
        if (eb_len > (ef_len - EB_HEADSIZE)) {
            /* discovered some extra field inconsistency! */
            Trace((stderr, "getUnicodeData: block length %u > rest ef_size %u\n", eb_len, ef_len - EB_HEADSIZE));
            break;
        }
        if (eb_id == EF_UNIPATH) {
            int offset = EB_HEADSIZE;
...
static int utf8_to_ucs4_string OF((ZCONST char* utf8, ulg* ucs4buf, int buflen));
            ...
            case EF_TIME:
                        ...
                        if ((eb_idx + 4) <= eb_len) {
                            ...
                            if ((ulg)(i_time) & (ulg)(0x80000000L)) {
                                if (dos_mdatetime == DOSTIME_MINIMUM) {
                                    ut_in_archive_sgn = -1;
                                    z_utim->mtime = (time_t)((long)i_time | (~(long)0x7fffffffL));
                                }
                                    ...
                                    TTrace((stderr, "  UT modtime range error; ignore e.f.!\n"));
                            ...
                            if ((ulg)(i_time) & (ulg)(0x80000000L)) {
                                ut_zip_unzip_compatible = ((time_t)0x80000000L < (time_t)0L) ? (dos_mdatetime == DOSTIME_MINIMUM) : (dos_mdatetime >= DOSTIME_2038_01_18);
                        ...
                        if ((eb_idx + 4) <= eb_len) {
                            ...
                            if ((ulg)(i_time) & (ulg)(0x80000000L)) {
                                if (ut_in_archive_sgn == -1)
                                    z_utim->atime = (time_t)((long)i_time | (~(long)0x7fffffffL));
                            }
                                ...
                                TTrace((stderr, "  UT access time range error: skip time!\n"));
                            }
                        }
                        else {
...
#else  /* !TIME_T_TYPE_DOUBLE */
                            if (((ulg)(i_time) & (ulg)(0x80000000L)) && !ut_zip_unzip_compatible) {
                                flags &= ~EB_UT_FL_ATIME;
                                TTrace((stderr, "  UT access time range error: skip time!\n"));
                            }
                            else {
                ...
                if (flags & EB_UT_FL_CTIME) {
                    if ((eb_idx + 4) <= eb_len) {
                        ...
                        if ((ulg)(i_time) & (ulg)(0x80000000L)) {
                            if (ut_in_archive_sgn == -1)
                                z_utim->ctime = (time_t)((long)i_time | (~(long)0x7fffffffL));
                        }
                        else if (ut_in_archive_sgn == 1) {
                            z_utim->ctime = (time_t)((ulg)i_time & (ulg)0xffffffffL);
                        }
                        else {
                            /* sign of 32-bit time is unknown -> ignore it */
                            flags &= ~EB_UT_FL_CTIME;
                            TTrace((stderr, "  UT creation time range error: skip time!\n"));
                        }
                    }
                    else {
    ...
    case EF_IZUNIX3:
        ...
        */
...
#ifdef IZ_HAVE_UXUIDGID
        if (eb_len >= EB_UX3_MINLEN && z_uidgid != NULL && (*((EB_HEADSIZE + 0) + ef_buf) == 1))
        /* only know about version 1 */
        {
            ...
            if (read_ux3_value((EB_HEADSIZE + 2) + ef_buf, uid_size, &z_uidgid[0]) && read_ux3_value((EB_HEADSIZE + uid_size + 3) + ef_buf, gid_size, &z_uidgid[1])) {
                flags |= EB_UX2_VALID; /* signal success */
            }
        }
#endif /* IZ_HAVE_UXUIDGID */
                ...
                if ((ulg)(i_time) & (ulg)(0x80000000L)) {
                    if (dos_mdatetime == DOSTIME_MINIMUM) {
                        ...
                        /* cannot determine sign of mtime;
                           without modtime: ignore complete UT field */
                        flags &= ~0x0ff; /* no time_t times available */
                        TTrace((stderr, "  UX modtime range error: ignore e.f.!\n"));
                    }
                }
                else {
                    /* cannot determine, safe assumption is FALSE */
...
#else  /* !TIME_T_TYPE_DOUBLE */
                        if (((ulg)(i_time) & (ulg)(0x80000000L)) && !ut_zip_unzip_compatible && (flags & 0x0ff)) {
