...
zvoid far** search OF((ZCONST zvoid*, ZCONST zvoid far**, extent, int (*)(ZCONST zvoid*, ZCONST zvoid far*)));
...
size_t bfwrite OF((ZCONST void* buffer, size_t size, size_t count, int));
...
#define o_NON_OPTION_ARG                               \
    ((unsigned long)0xFFFF) /* returned for non-option \
                               args */
#define o_ARG_FILE_ERR                            \
    ((unsigned long)0xFFFE) /* internal recursion \
...
unsigned long get_option OF((char*** pargs, int* argc, int* argnum, int* optchar, char** value, int* negated, int* first_nonopt_arg, int* option_num, int recursion_depth));
...
int insert_arg OF((char*** args, ZCONST char* arg, int insert_at, int free_args));
