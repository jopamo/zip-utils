...
/* y is now global */
{
    ...
    */
    ...
    if (strcmp(z->name, "-") == 0) {
        ...
        if (!translate_eol && m != STORE && q != -1L && (ulg)q > 0 && (ulg)q + MIN_LOOKAHEAD > (ulg)q) {
#ifdef MMAP
            ...
            if (remain > MIN_LOOKAHEAD) {
                ...
                if (window != NULL) {
                    window = (uch*)mmap((char*)window, window_size - remain, PROT_READ, MAP_PRIVATE | MAP_FIXED, ifile, 0);
                }
                else {
                    window = (uch*)(-1);
                }
            ...
            window_size = (ulg)q + MIN_LOOKAHEAD;
            window = window ? (uch*)realloc(window, (unsigned)window_size) : (uch*)malloc((unsigned)window_size);
    ...
    if (o != -1 && last_o > o) {
        fprintf(mesg, "last %s o %s\n", zip_fzofft(last_o, NULL, NULL), zip_fzofft(o, NULL, NULL));
    ...
    if (isdir) {
...
#endif
        {
            ...
            if (bytes_this_entry != (uzoff_t)(key ? s + 12 : s)) {
                fprintf(mesg, " s=%s, actual=%s ", zip_fzofft(s, NULL, NULL), zip_fzofft(bytes_this_entry, NULL, NULL));
      ...
      if (tempzn - o != s) {
        fprintf(mesg, " s=%s, actual=%s ",
                zip_fzofft(s, NULL, NULL), zip_fzofft(tempzn - o, NULL, NULL));
        error("incorrect compressed size");
      }
#endif /* !VMS && !CMS_MVS */
#endif /* 0 */
    ...
    if (noisy) {
        if (verbose) {
            fprintf(mesg, "\t(in=%s) (out=%s)", zip_fzofft(isize, NULL, "u"), zip_fzofft(s, NULL, "u"));
        }
#ifdef BZIP2_SUPPORT
        if (m == BZIP2)
 ...
 */
{
    ...
    if (remain == 0L) {
        ...
        if ((ulg)size > remain)
            size = (unsigned)remain;
        if (size > WSIZE)
            size = WSIZE; /* don't touch all pages at once */
        remain -= (ulg)size;
        len = size;
    }
    else
...
int pack_level;
{
    ...
    if (zlib_version[0] != ZLIB_VERSION[0]) {
        sprintf(errbuf, "incompatible zlib version (expected %s, found %s)", ZLIB_VERSION, zlib_version);
        zp_err = ZE_LOGIC;
    }
    else if (strcmp(zlib_version, ZLIB_VERSION) != 0) {
        fprintf(mesg, "\twarning:  different zlib version (expected %s, using %s)\n", ZLIB_VERSION, zlib_version);
    }
 ...
 */
void flush_outbuf(o_buf, o_idx) char* o_buf;
unsigned* o_idx;
{
    if (y == NULL) {
    ...
    if (*o_idx != 0) {
        zfwrite(o_buf, 1, (extent)*o_idx, y);
        if (ferror(y))
...
#define OBUF_SZ ZBSZ
    ...
    if (zstrm.avail_in < ibuf_sz) {
        unsigned more = file_read(zstrm.next_in + zstrm.avail_in, (ibuf_sz - zstrm.avail_in));
        if (more == EOF || more == 0) {
            maybe_stored = TRUE;
        }
        else {
    ...
    if (!maybe_stored)
        while (zstrm.avail_in != 0 && zstrm.avail_in != EOF) {
            ...
            if (zstrm.avail_out == 0) {
                if (zfwrite(f_obuf, 1, OBUF_SZ, y) != OBUF_SZ) {
    ...
    do {
        ...
        if (maybe_stored) {
            if (err == Z_STREAM_END && zstrm.total_out >= zstrm.total_in && fseekable(zipfile)) {
                /* deflation does not reduce size, switch to STORE method */
                unsigned len_out = (unsigned)zstrm.total_in;
                if (zfwrite(f_ibuf, 1, len_out, y) != len_out) {
                    ziperr(ZE_TEMP, "error writing to zipfile");
                }
        ...
        if (zstrm.avail_out < OBUF_SZ) {
            ...
            if (zfwrite(f_obuf, 1, len_out, y) != len_out) {
                ziperr(ZE_TEMP, "error writing to zipfile");
            }
            zstrm.next_out = (Bytef*)f_obuf;
            zstrm.avail_out = OBUF_SZ;
...
#define OBUF_SZ ZBSZ
    ...
    if (bstrm.avail_in < ibuf_sz) {
        unsigned more = file_read(bstrm.next_in + bstrm.avail_in, (ibuf_sz - bstrm.avail_in));
        ...
        while (bstrm.avail_in != 0 && bstrm.avail_in != (unsigned)EOF) {
            ...
            if (bstrm.avail_in == 0) {
                if (verbose || noisy)
                    ...
                    while ((unsigned)((bstrm.total_in_lo32 + (((zoff_t)bstrm.total_in_hi32) << 32)) / (zoff_t)(ulg)WSIZE) > mrk_cnt) {
#else
    ...
    do {
        ...
        if (maybe_stored) {
            /* This code is only executed when the complete data stream fits
               ...
               limit.
             */
            if (err == BZ_STREAM_END && bstrm.total_out_lo32 >= bstrm.total_in_lo32 && fseekable(zipfile)) {
    ...
    if (z_entry->att == (ush)UNKNOWN)
        z_entry->att = (ush)BINARY;
#ifdef LARGE_FILE_SUPPORT
    cmpr_size = (zoff_t)bstrm.total_out_lo32 + (((zoff_t)bstrm.total_out_hi32) << 32);
#else
