...
/* For the encoding task used in Zip (and ZipCloak), we want to initialize
   ...
   the crypthead() function.

   On Unix we try /dev/urandom first. If that fails, we fall back to
   srand()/rand() using a mixed seed (time() XOR ZCR_SEED2).

   NOTE:
   1.) This implementation requires time(), rand(), srand() for the fallback.
   2.) Do NOT call time() twice for both seed parts; we deliberately XOR a
       second, independent value (ZCR_SEED2).
 */
#include <time.h>
#ifndef ZCR_SEED2
#define ZCR_SEED2 (unsigned)3141592654L /* PI as last-resort pattern */
#endif
...
int update_keys(__G__ c) __GDEF int c; /* byte of plain text */
{
    ...
    GLOBAL(keys[1]) = (GLOBAL(keys[1]) + (GLOBAL(keys[0]) & 0xff)) * 134775813L + 1;
    {
 ...
 * the given password.
 */
void init_keys(__G__ passwd) __GDEF ZCONST char* passwd; /* password string with which to modify keys */
{
#ifdef IZ_CRC_BE_OPTIMIZ
    if (cry_crctb_p == NULL) {
 ...
 * See comment in original source.
 */
#ifdef IZ_CRC_BE_OPTIMIZ
local z_uint4 near* crytab_init(__G) __GDEF {
    int i;
 ...
 *
 * FIX: Only a single encryption pass over the header (no double-encrypt).
 *      We now generate RAND_HEAD_LEN-2 raw random bytes, then initialize
...
FILE* zfile;                                            /* where to write header */
{
    ...
    uch header[RAND_HEAD_LEN]; /* random header (plaintext then encrypted) */
    ...
    if (unix_fill_random(header, RAND_HEAD_LEN - 2) != 0) {
        ...
        for (n = 0; n < RAND_HEAD_LEN - 2; n++) {
            header[n] = (uch)((rand() >> 7) & 0xff);
        }
    }

    /* Append the two check bytes (plaintext) = high CRC word (or time) */
#ifdef ZIP10
    /* historical two-byte check (older ZIP) – keep behaviour if enabled */
    header[RAND_HEAD_LEN - 2] = (uch)((crc >> 16) & 0xff);
    header[RAND_HEAD_LEN - 1] = (uch)((crc >> 24) & 0xff);
#else
    /* pkzip 2.0+: only the MSB is used for the check, but traditional code
       stores both bytes from the high CRC word before encrypting. */
    header[RAND_HEAD_LEN - 2] = (uch)((crc >> 16) & 0xff);
    header[RAND_HEAD_LEN - 1] = (uch)((crc >> 24) & 0xff);
#endif

    /* 2) Initialize keys and encrypt the entire header exactly once. */
    init_keys(passwd);
...
ZCONST char* passwd; /* password string */
{
    ...
    if (fseek(source, (long)((4 + LOCHEAD) + (ulg)z->nam + (ulg)z->ext), SEEK_CUR)) {
...
ZCONST char* passwd; /* password string */
{
    ...
    if ((offset = (ulg)ftell(source)) == (ulg)-1L || fseek(source, (long)((4 + LOCHEAD) + (ulg)z->nam + (ulg)z->ext), SEEK_CUR)) {
        return ferror(source) ? ZE_READ : ZE_EOF;
    }
...
#ifdef ZIP10
    if ((ush)(c0 | (c1 << 8)) != (z->flg & 8 ? (ush)z->tim & 0xffff : (ush)(z->crc >> 16))) {
#else
    if ((ush)c1 != (z->flg & 8 ? (ush)z->tim >> 8 : (ush)(z->crc >> 24))) {
        ...
        if ((res = zipcopy(z, source, dest)) != ZE_OK)
            return res;
        return ZE_MISS;
    }
    ...
    /* Clear encrypted bit and local header bit, and write local header to
       output file */
    if ((offset = (ulg)ftell(dest)) == (ulg)-1L)
        return ZE_TEMP;
    z->off = offset;
    flag = z->flg;
    z->flg &= ~9;
...
int decrypt(__G__ passwrd) __GDEF ZCONST char* passwrd;
{
    ...
    if (GLOBAL(newzip)) { /* this is first encrypted member in this zipfile */
        ...
        if (passwrd != (char*)NULL) { /* user gave password on command line */
            if (!GLOBAL(key)) {
                if ((GLOBAL(key) = (char*)malloc(strlen(passwrd) + 1)) == (char*)NULL)
                    return PK_MEM2;
    ...
    n = 0;
    do {
        r = (*G.decr_passwd)((zvoid*)&G, &n, GLOBAL(key), IZ_PWLEN + 1, GLOBAL(zipfn), GLOBAL(filename));
...
ZCONST char* key; /* decryption password to test */
{
    ...
    if ((ush)(c | (b << 8)) != (GLOBAL(pInfo->ExtLocHdr) ? ((ush)GLOBAL(lrec.last_mod_dos_datetime) & 0xffff) : (ush)(GLOBAL(lrec.crc32) >> 16)))
        return -1; /* bad */
#else
    ...
    for (n = (long)GLOBAL(incnt) > GLOBAL(csize) ? (int)GLOBAL(csize) : GLOBAL(incnt), p = GLOBAL(inptr); n--; p++)
        zdecode(*p);
    return 0; /* OK */
